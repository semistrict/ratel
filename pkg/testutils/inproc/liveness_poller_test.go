...
// TestSyncFirstRangeUpdates verifies that the first-range update loop
// (which uses time.NewTicker) runs under synctest fake time. After
// advancing past the 30s interval, the first range descriptor should
// still be available via the shared provider.
func TestSyncFirstRangeUpdates(t *testing.T) {
	synctest.Test(t, func(t *testing.T) {
		c := startSyncCluster(t, 3)
		defer stopSyncCluster(c)

		// First range should be available immediately after cluster start.
		desc, err := c.Server(0).FirstRangeProviderI().(interface {
		...
		// Advance past the 30s first-range update ticker interval.
		time.Sleep(35 * time.Second)
		synctest.Wait()

		// First range should still be available after the ticker fires.
		desc2, err := c.Server(0).FirstRangeProviderI().(interface {
			GetFirstRangeDescriptor() (*roachpb.RangeDescriptor, error)
		}).GetFirstRangeDescriptor()
		require.NoError(t, err)
		require.NotNil(t, desc2)
		require.Equal(t, roachpb.RangeID(1), desc2.RangeID)
	})
}
