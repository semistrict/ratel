...
func spanGeneratorOracle(
	...
	for _, inputRow := range rows {
		...
		spans = spanSplitter.AppendSpan(
			spans, generatedSpan, lookupCols, containsNull)
	}
	return spans
}

func makeTable() (catalog.TableDescriptor, error) {
	var testTableDesc = descpb.TableDescriptor{
		Name:       "abcd",
		ID:         descpb.ID(bootstrap.TestingUserDescID(0)),
		...
		RowGroups: []descpb.RowGroupDescriptor{
			{Name: "primary", ID: 0, ColumnNames: []string{"a", "b", "c", "d"}, ColumnIDs: []descpb.ColumnID{1, 2, 3, 4}},
		},
		NextRowGroupID: 1,
