...
// TestMultiNodeCompaction writes enough data across nodes to generate
// SSTables, forces flush and compaction on each, then verifies all data is
// still readable. This is designed to catch CreatorID collisions (all nodes
// sharing the same Pebble CreatorID would produce identical SSTable
// filenames, causing overwrites on shared storage).
...
// and verifies that cross-node reads still succeed. If the per-store
// CreatorID fix regresses (all nodes back to CreatorID 1) or the per-store
// MANIFEST bundle naming regresses, the reads here will fail with
// "object does not exist" during compaction.
func TestSharedMetadataCorruption(t *testing.T) {
	...
	defer tc.Stopper().Stop(ctx) //nolint:govet // cluster must stop before ss.Close() below
