...
// backed by in-memory remote storage.
package rateltest
...
// metadata store (since the Pebble MANIFEST is per-node) but shares the
// SSTable store.
...
// storage wrapped in a no-close shim. Pebble v1.1.5's in-memory store zeros
// itself on Close, so without the shim the first engine to shut down would
// wipe the shared state for the remaining engines.
func (ss *SharedStorage) factoryFor() remote.StorageFactory {
	return &fixedFactory{store: noCloseStorage{ss.SSTables}}
}
...
// noCloseStorage is a remote.Storage that drops Close calls on the floor.
// Intended for sharing a single underlying in-memory store across multiple
// Pebble engines in tests.
type noCloseStorage struct {
	remote.Storage
}
...
// so that the remote storage factory is actually wired into Pebble —
// in-memory stores skip remote storage configuration.
