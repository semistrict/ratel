...
`)
	...
	err := sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn isql.Txn, col *descs.Collection) error {
		funcDesc, err := col.ByIDWithLeased(txn.KV()).WithoutNonPublic().Get().Function(ctx, 114)
...
SELECT nextval(109:::REGCLASS);`,
		...
		require.Equal(t,
			[]descpb.TableDescriptor_Reference{
				{ID: 114, ColumnIDs: []catid.ColumnID{1}},
				{ID: 114, IndexID: 2, ColumnIDs: []catid.ColumnID{2}},
				{ID: 114, IndexID: 3, ColumnIDs: []catid.ColumnID{3}},
			},
			tbl.GetDependedOnBy(),
		)
		...
		require.Equal(t, descpb.ID(114), typ.GetReferencingDescriptorID(0))
...
func TestCreateOrReplaceFunctionUpdateReferences(t *testing.T) {
	...
	validateReferences := func(
		...
		require.NoError(t, err)
		require.Equal(t,
			[]descpb.TableDescriptor_Reference{{ID: 116, IndexID: 2, ColumnIDs: []catid.ColumnID{2}}},
			tbl.GetDependedOnBy())
		...
		require.NoError(t, err)
		require.Equal(t, []descpb.TableDescriptor_Reference{{ID: 116, ByID: true}}, seq.GetDependedOnBy())
		...
		require.NoError(t, err)
		require.Equal(t,
			[]descpb.TableDescriptor_Reference{{ID: 116, ColumnIDs: []catid.ColumnID{1}}},
			view.GetDependedOnBy())
...
`,
	...
	err := sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn isql.Txn, col *descs.Collection) error {
		funcDesc, err := col.ByIDWithLeased(txn.KV()).WithoutNonPublic().Get().Function(ctx, 116)
		require.NoError(t, err)
		require.Equal(t, funcDesc.GetName(), "f")
...
SELECT nextval(110:::REGCLASS);`,
			funcDesc.GetFunctionBody())

		sort.Slice(funcDesc.GetDependsOn(), func(i, j int) bool {
			return funcDesc.GetDependsOn()[i] < funcDesc.GetDependsOn()[j]
		})
		require.Equal(t, []descpb.ID{108, 110, 112}, funcDesc.GetDependsOn())
		sort.Slice(funcDesc.GetDependsOnTypes(), func(i, j int) bool {
			return funcDesc.GetDependsOnTypes()[i] < funcDesc.GetDependsOnTypes()[j]
		})
		require.Equal(t, []descpb.ID{114, 115}, funcDesc.GetDependsOnTypes())
		...
		require.NoError(t, err)
		require.Equal(t, 1, typ.NumReferencingDescriptors())
		require.Equal(t, descpb.ID(116), typ.GetReferencingDescriptorID(0))
...
`)
	...
	err = sql.TestingDescsTxn(ctx, s, func(ctx context.Context, txn isql.Txn, col *descs.Collection) error {
		funcDesc, err := col.ByID(txn.KV()).WithoutNonPublic().Get().Function(ctx, 116)
		require.NoError(t, err)
		require.Equal(t, funcDesc.GetName(), "f")
...
SELECT nextval(111:::REGCLASS);`,
		...
		require.Equal(t, []descpb.ID{109, 111, 113}, funcDesc.GetDependsOn())
		sort.Slice(funcDesc.GetDependsOnTypes(), func(i, j int) bool {
			return funcDesc.GetDependsOnTypes()[i] < funcDesc.GetDependsOnTypes()[j]
		})
		require.Equal(t, []descpb.ID{114, 115}, funcDesc.GetDependsOnTypes())
