...
func (n *createWasmFunctionNode) startExec(params runParams) error {
	...
	for i, param := range n.n.Params {
		...
		vt, err := wasmruntime.SQLTypeToValType(t)
		if err != nil {
			return err
		}
	...
	retValType, err := wasmruntime.SQLTypeToValType(retType)
	if err != nil {
		return err
	}

	// Compile WAT to WASM.
	wasmBytes, err := wasmruntime.Wat2Wasm(n.n.Body)
	if err != nil {
		return fmt.Errorf("compiling WAT: %w", err)
	}

	// Get the WASM registry from server config.
	registry := p.execCfg.WasmRegistry
	if registry == nil {
		return fmt.Errorf("WASM runtime not available on this node")
	}

	funcName := string(n.n.Name)

	// Register in the WASM runtime.
	err = registry.CompileAndRegister(ctx, funcName, wasmBytes, "invoke",
	...
	if err := registerWasmFunDef(registry, funcName, sqlArgTypes, retType, n.n.Volatility); err != nil {
		registry.Deregister(ctx, funcName)
		return err
	}
...
// registerWasmFunDef creates a FunctionDefinition from a compiled WASM
// function and registers it in the global FunDefs map.
func registerWasmFunDef(
	registry *wasmruntime.Registry,
	funcName string,
	sqlArgTypes []*types.T,
	retType *types.T,
	...
	overload := tree.Overload{
		...
		Info:             fmt.Sprintf("WASM user-defined function %s", funcName),
	...
	def := tree.NewFunctionDefinition(
		...
		&tree.FunctionProperties{
			Category:                "User-defined WASM functions",
...
func encodeArgTypes(types []*types.T) []byte {
	...
	for i, t := range types {
		vt, _ := wasmruntime.SQLTypeToValType(t)
		result[i] = byte(vt)
	}
	return result
}
